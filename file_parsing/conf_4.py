...
# Selecting the required columns including the new clone_count
...
msub_no_stop_codons = merged_al_cleaned.filter(stop_codon_heavy="F", stop_codon_light="F")
...
# Aggregating HCDR3s using the actual clone_count from vsearch
all_hcdr3s = msub_null_clean.group_by("cdr3_aa_heavy").agg(
    pl.col("clone_count").sum().alias("count")
).sort("count", descending=True)

all_hcdr3s.write_csv("all_hcdr3s.csv")

top_20_hcdr3s = all_hcdr3s.head(20)
print(top_20_hcdr3s)
...
# Aggregating LCDR3s using the actual clone_count
top_20_lcdr3s = msub_null_clean.group_by("cdr3_aa_light").agg(
    pl.col("clone_count").sum().alias("count")
...
path = Path(work_dir) / "igblast_tsv_files"
...
# Looking for multiple light chains within each of the HCDR3 groups
...
for fasta in heavy_fasta.glob("*.fasta"):
    ...
    with open(fasta, 'r') as fa:
        for line in fa:
            if line.startswith(">"):
                # Extract size from header and select 1 if not present
    ...
    if total_abundance > count_cut_off:
        fasta_above_cut_off += 1

print(f"There are {fasta_above_cut_off} files with more than {count_cut_off} sequences (weighted by true count)")
