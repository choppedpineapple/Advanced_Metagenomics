...
# FIX: Single quotes inside the f-string to prevent syntax errors on older Python versions
print(f"Memory: {msub_null_clean.estimated_size('mb'):.2f} megabytes")

# FIX: Create the directory BEFORE you start trying to write files to it!
...
for tsv in path.glob("*.tsv"):
   ...
   for i, seq in enumerate(tsv_df.iter_rows(named=True)):
       ...
       # FIX: Swapped to single quotes for dictionary keys inside f-strings
   ...
   for i, seq in enumerate(tsv_df.iter_rows(named=True)):
       ...
       # FIX: Swapped to single quotes here too
...
# FIX: Swapped to single quotes inside glob
